...
const VARIANT_COLORS: Record<VoiceOrbVariant, [number, number, number][]> = {
  ...
  // Adyatech brand — matched to the button colors:
  // base --brand-red (#D00000), bright veins --brand-red-3, depth --brand-red-2 (#A80000)
  brand: [
    [0.82, 0.0, 0.0],
    [0.95, 0.08, 0.08],
    [0.6, 0.0, 0.0],
...
// navbar text color — #F5F2EA
const vec3 CREAM = vec3(0.961, 0.949, 0.918);
...
void main() {
  ...
  col += vein * mix(u_color1, CREAM, 0.2);

  float centerDist = dist / radius;
  float depthShade = 1.0 - centerDist * centerDist * 0.25;
  col *= depthShade;

  float rim = pow(centerDist, 4.0) * 0.6;
  col += rim * mix(u_color0, CREAM, 0.4);
